 ...
 * @author jbannon
 */
public class Sine extends Function_Properties {
    ...
    public Sine(final String theEquation) throws ParseException {
        ...
        for (char c : logArray) {
            ...
            if (Character.isDigit(c) && !verified) {
            ...
            else if ((c == 's' || c == 'S') && !verified && index + 3 < logArray.length
                && (logArray[index + 1] == 'i' || logArray[index + 1] == 'I')
                ...
                throw new ParseException("Invalid sin at index " + index, index);
    ...
    public double value(final ArrayList<Variable> theVariable) {
        try {
            ...
            return constant * Math.sin(contents.solveEquation(theVariable));
        } catch (ParseException ex) {
            Logger.getLogger(Sine.class.getName()).log(Level.SEVERE, null, ex);
            return Double.NaN;
        }
    }
